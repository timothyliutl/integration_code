...
for alpha in [0.5]:
    ...
    x_val = np.arange(-6,6,0.01)
    y_val = np.arange(-6,6,0.01)
    z_val = np.arange(-6,6,0.01)
...
#for alpha in [0.99]:
    ...
    inner_int_matrix_ajs[blockspergrid, threadsperblock](x_val, y_val, z_val, 0.01, convolution_x_val[0] ,alpha, convolution_array_w1w2, convolution_array_yw1w2, convolution_array_yw1, result_matrix)
    np.savetxt("fy_z.csv", result_matrix[500,:,500], delimiter=",")
    #reduce_matrix = result_matrix.reshape(-1)

    #final_result = gpu_sum_reduce(reduce_matrix)*(0.01**3)
